        ...
        <header className="sticky top-0 z-30 bg-white/80 backdrop-blur-md border-b border-slate-200 px-8 py-4 flex justify-between items-center">
          <div className="flex items-center gap-4">
             ...
             <Button 
                ...
                variant="ghost" 
                size="icon" 
                className="h-8 w-8 rounded-lg text-slate-400 hover:text-unda-navy hover:bg-slate-100 hidden md:flex"
                title="Go Back"
             >
                <ArrowLeft size={18} />
             </Button>
             ...
             <div>
                <h1 className="text-xl font-bold text-unda-navy leading-none">{title}</h1>
          ...
          <div className="flex items-center gap-3">
            <div className="hidden md:flex relative group">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-unda-teal transition-colors" size={16} />
              <input 
                type="text" 
                placeholder="Search..." 
                className="h-9 pl-9 pr-4 rounded-full bg-slate-100 border-transparent text-sm focus:bg-white focus:outline-none focus:border-unda-teal focus:ring-2 focus:ring-unda-teal/10 w-48 focus:w-64 transition-all duration-300"
              />
            </div>
            
            <div className="h-6 w-px bg-slate-200 mx-2 hidden md:block"></div>

            <Button size="icon" variant="ghost" className="relative h-9 w-9 rounded-full text-slate-500 hover:text-unda-navy hover:bg-slate-100">
              <Bell size={18} />
            ...
            <div className="flex items-center gap-3 pl-2 border-l border-slate-200 ml-2">
              <div className="text-right hidden sm:block">
                <p className="text-sm font-bold text-unda-navy leading-none">{displayName}</p>
                <p className="text-[10px] text-slate-500 font-medium capitalize">{displayRole}</p>
              </div>
              <div className="h-9 w-9 rounded-full bg-gradient-to-br from-unda-teal to-teal-600 flex items-center justify-center text-white font-bold text-xs ring-4 ring-slate-50 shadow-sm cursor-pointer hover:ring-unda-teal/20 transition-all">
                {initials}
              </div>
            </div>
          </div>
